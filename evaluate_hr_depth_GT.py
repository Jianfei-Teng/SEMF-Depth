...
import warnings
# Filter warnings immediately after future imports to catch early library loads
...
def evaluate(opt, epoch=None):
    ...
    # [FIX] Use dynamic project root to avoid 'Datapath not found' errors
    eval_data_root = os.path.join(project_root, "gt_depth")
    
    # Dynamically find scene folders ("1", "2", ..., "11" etc.)
    if os.path.exists(eval_data_root):
        scenes = sorted([d for d in os.listdir(eval_data_root) if os.path.isdir(os.path.join(eval_data_root, d))],
                        key=lambda x: int(x) if x.isdigit() else x)
    else:
        print(f"[Error] Datapath not found: {eval_data_root}")
        # Always return a list of dictionaries to prevent crashes in result saving
        return [{"scene": "ERROR", "metrics": [0.0] * 7}]

    all_errors = []
    all_ratios = []
    per_scene_results = []

    print(f"\n-> Start scene-by-scene evaluation from {eval_data_root} ...")
    ...
    for scene in scenes:
        ...
        for i, img_path in enumerate(valid_paths):
            ...
            if not os.path.exists(gt_path):
                print(f"[Warning] GT npy file not found for: {img_path}")
                continue
            ...
            all_errors.append((errs, img_path))
...
def save_per_scene_results_to_txt(all_results, project_root, stage_name):
    ...
    txt_path = os.path.join(project_root, f"hr_evaluation_per_scene_{stage_name}_custom.txt")
    ...
    # Sort numeric sort for scenes "1", "2" ... and "ALL" at end
    scene_list = sorted([s for s in scene_set if s != "ALL"], key=lambda x: int(x) if x.isdigit() else x)
    if "ALL" in scene_set:
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("\n" + "=" * 95 + "\n")
        f.write(f" [{timestamp}] STAGE {stage_name} HR EVALUATION SCENE-BY-SCENE (CUSTOM DATA)\n")
        f.write("=" * 95 + "\n")
