...
# @BACKBONES.register_module()  # Removed for standalone use
...
def mpvit_xsmall(**kwargs):
    ...
    checkpoint = torch.load('./ckpt/mpvit_xsmall.pth', map_location=lambda storage, loc: storage, weights_only=False)['model']
    logger = get_root_logger()
...
def mpvit_small(**kwargs):
    ...
    pretrained_path = kwargs.get('pretrained', './ckpt/mpvit_small.pth')
    if pretrained_path and os.path.exists(pretrained_path):
        print(f"Loading pretrained model from {pretrained_path}")
        checkpoint = torch.load(pretrained_path, map_location='cpu', weights_only=False)
        
        # Handle 'model' key if present (common in timm/mmcv)
        ...
        # Use native load_state_dict to get missing/unexpected keys
        msg = model.load_state_dict(state_dict, strict=False)
        
        # Verify "All Information" loaded (except head)
        filtered_missing = [k for k in msg.missing_keys if not k.startswith('head')]
        if len(filtered_missing) > 0:
             print(f"[WARNING] Missing keys in pretrained: {filtered_missing}")
             # User said "Load ALL information ... other do not move".
             # If significant keys missing, strict requirement suggests checking.
             # However, timm models might have different naming.
             # We print warning but proceed if user insists "other don't move".
             # Actually, let's trust the load. 
        else:
             print("[INFO] All backbone weights loaded successfully.")
